...
#for location
        ...
        os.environ["ENV_VAR"] = self._save_location
...
#-------------------------------Code to submit details to database-----------------------------------------#
try:
    ...
    plugin_folder_name = os.path.basename(script_directory).split('.')[0]
    print("Plugin Folder Name: {}".format(plugin_folder_name))

    url = "https://api.arth.team/api/v1/revit/"
    data = {
        'name':app.Username,
